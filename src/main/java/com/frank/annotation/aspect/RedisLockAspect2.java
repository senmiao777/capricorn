...
public class RedisLockAspect2 {
    ...
    public void around(ProceedingJoinPoint joinPoint) {
        try {
            log.info("[RedisLockAspect2] ---before---");
            ...
            try {
                ...
                log.error("[RedisLockAspect2] redis Exception e={}", ExceptionUtils.getStackTrace(e));
            }
            /**
            ...
            log.info("[RedisLockAspect2] ---after---lockKey={}", lockKey);
            try {
                final Boolean expire = redisTemplate.expire(lockKey, ZERO, TimeUnit.MILLISECONDS);
                log.info("[RedisLockAspect2] ---after---lockKey={},expire={}", lockKey, expire);
            } catch (Exception e) {
                log.error("[RedisLockAspect2] after expire Exception.lockKey={}e={}", lockKey, ExceptionUtils.getStackTrace(e));
            }
        } catch (Throwable throwable) {
            /**
             * 这里执行的操作，相当于在@AfterThrowing里写的操作
             */
            log.error("[RedisLockAspect2][AfterThrowing] e={}", ExceptionUtils.getStackTrace(throwable));
        }
    ...
    private String getLockKey(JoinPoint joinPoint) {
        ...
        if (key.startsWith(PREFIX_SPEL)) {
            try {
                ...
                log.error("[RedisLockAspect2]spel Exception.input spel key={},e={}", key, ExceptionUtils.getStackTrace(e));
                throw e;
            }
        } else {
